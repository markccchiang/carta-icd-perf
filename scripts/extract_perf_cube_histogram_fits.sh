...
for logfile in "$LOG_DIR"/perf-*.log; do
    ...
    if ! grep -q "PASS src/performance/PERF_CUBE_HISTOGRAM_FITS.test.ts" "$logfile"; then
        continue
    fi

    # Extract the elapsed time from the Step 2 line after the PASS line
    elapsed=$(awk '
        /PASS src\/performance\/PERF_CUBE_HISTOGRAM_FITS\.test\.ts/ { found=1; next }
        found && /\(Step 2\).*cube_B_06400_z00100\.fits.*REGION_HISTOGRAM_DATA should arrive completely within 300000 ms:/ {
            n = split($0, a, "(")
            for (i = 1; i <= n; i++) {
                if (a[i] ~ /^[0-9]+ ms\)/) {
                    sub(/ ms\).*/, "", a[i])
