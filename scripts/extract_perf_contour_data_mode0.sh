...
OUTPUT_FILE="$BASE_DIR/test_logs/PERF_CONTOUR_DATA_Mode0.log"
...
for logfile in "$LOG_DIR"/perf-*.log; do
    ...
    elapsed=$(awk '
        /PASS src\/performance\/PERF_CONTOUR_DATA_Mode0\.test\.ts/ { found=1; next }
        found && /\(Step 2\) smoothingMode of 0 ContourImageData responses should arrive within 12000 ms/ {
            n = split($0, a, "(")
            for (i = 1; i <= n; i++) {
                if (a[i] ~ /^[0-9]+ ms\)/) {
                    sub(/ ms\).*/, "", a[i])
