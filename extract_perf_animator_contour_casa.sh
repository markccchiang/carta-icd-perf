...
LOG_DIR="log"
OUTPUT_FILE="test_logs/PERF_ANIMATOR_CONTOUR_CASA.log"
...
for logfile in "$LOG_DIR"/perf-*.log; do
    ...
    if ! grep -q "PASS src/performance/PERF_ANIMATOR_CONTOUR_CASA.test.ts" "$logfile"; then
        printf "%-14s%-52s%s\n" "$formatted_date" "PERF_ANIMATOR_CONTOUR_CASA" "N/A" >> "$OUTPUT_FILE"
        continue
    fi
    ...
    if [ -n "$elapsed" ]; then
        printf "%-14s%-52s%s ms\n" "$formatted_date" "PERF_ANIMATOR_CONTOUR_CASA" "$elapsed" >> "$OUTPUT_FILE"
    else
        printf "%-14s%-52s%s\n" "$formatted_date" "PERF_ANIMATOR_CONTOUR_CASA" "N/A" >> "$OUTPUT_FILE"
    fi
done
